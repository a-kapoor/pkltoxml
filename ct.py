...
class Tree:
    ...
    children : list of int
        The id-s of the children associated with this node
    depth : int
    ...
    payload : tuple
        Describes what this node does, i.e. is it a non-terminal node(cut) or
        a terminal (leaf)
    """
    ...
    def __repr__(self):
        out_str = 'Tree id: %s children:%s parent:%s, depth:%s, payload:%s' \
            % (
                self.id, str(self.children), self.parent,
                self.depth, self.payload
            )
        return out_str

    def print_out(self, node_dict):
        """Recursively prints a node and its children, given a dictionary with
        all the available nodes
        Parameters:
        ----------
        node_dict : dict id->node
            All the available nodes
        Returns:
        -------
        Nothing
    ...
    def to_tmva(self, nodetree, scale, index_mapping):
        ...
        Parameters:
        ----------
        nodetree : (dict int->Tree)
            The dictionary of the full tree
        scale : float
            A scaling coefficient for the TMVA leaves (TMVA = sklearn * scale)
        Returns:
        -------
...
def sklearn_to_nodetree(
    ...
    """Recursively converts a sklearn GradientBoosting{Classifier,Regressor} to
    a generic representation
    Parameters:
    ----------
    nodetree : dict id->Node)
        The output dictionary with the nodes
    ...
    -------
    dict int : Tree
        The output node tree
    """
    # if the left (or right) child node id is -1, then this node is already a
    # leaf node
    ...
    # this is not a leaf node
        ...
        n = Tree(
            ...
            parent_id,
            depth,
            ("cut", sklearn_tree.feature[node_id],
             sklearn_tree.threshold[node_id])
        )
        nodetree[node_id] = n
        if parent_id in nodetree:
            nodetree[parent_id].children += [node_id]
    ...
    if left_child != _tree.TREE_LEAF:
        sklearn_to_nodetree(cls, nodetree, sklearn_tree,
                            left_child, node_id, depth+1)
    if right_child != _tree.TREE_LEAF:
        sklearn_to_nodetree(cls, nodetree, sklearn_tree,
                            right_child, node_id, depth+1)
...
def xgbtree_to_nodetree(tree):
    ...
    ----------
    tree : str
        The model dump from xgboost using model.get_booster().get_dump()[ntree]
    Returns:
    --------
    dict : int->Tree
        The tree structure
    ...
    for node in tree.split("\n"):
        ...
        if is_node:
            nodes[node_index] = Tree(
                node_index, [], my_parent, node_depth,
                ("cut", node_variable, threshold)
            )
        elif is_leaf:
            nodes[node_index] = Tree(
                node_index, [], my_parent, node_depth, ("val", val))
...
class BDT(object):
    def __init__(
            self, trees, kind, feature_names,
            target_names, max_depth, learning_rate
    ):
        self.trees = trees
        self.kind = kind
...
class BDTxgboost(BDT):
    def __init__(self, model, feature_names, target_names):
        ...
        for tree_dump in model_booster.get_dump():
            ...
            trees += [tree]
        super(BDTxgboost, self).__init__(
            trees, kind, feature_names,
            target_names, model.max_depth, model.learning_rate
        )

    def eval(self, features):
        proba = self.model.predict_proba(features)[:, 1]
        print("eval (before transformation) = " + str(proba))
        # invert sigmoid
        proba = -np.log(1.0/proba - 1.0)
        # apply TMVA transformation
        proba = 2.0 / (1.0 + np.exp(-2.0*proba)) - 1
        print("eval (After transformation) = " + str(proba))
        x = 1/(1+np.sqrt((1-proba)/(1+proba)))
        print("eval (After inverse transformation) = " + str(x))
        return proba
...
class BDTsklearn(BDT):
    def __init__(self, model, feature_names, target_names):
        ...
        if isinstance(model, GradientBoostingRegressor):
            ...
            if len(target_names) == 2:
                ...
                kind = "multiclass"
        trees = []
        # Loop over decision trees, in scikit that's a 2D array with a layout
        ...
        for sklearn_trees in model.estimators_:
            ...
            for class_tree in sklearn_trees:
                nodetree = {}
                sklearn_to_nodetree(
                    model, nodetree, class_tree.tree_, 0, -1, -1)
                trees += [nodetree]
        super(BDTsklearn, self).__init__(
            trees, kind, feature_names, target_names,
            model.max_depth, model.learning_rate
        )
    ...
    def eval(self, vals):
        ...
        ----------
        vals : numpy array
            An array (n_samples, n_features) of the input variables
        Returns:
        -------
        ret : numpy array
            (n_samples, n_classes) array of the output
        """
